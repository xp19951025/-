	...
	<form action="system/queryUsers" id="f1" class="form-inline" method="post">
		<div class="row alert alert-info" style="margin: 0px; padding: 5px;">
			<div class="form-group" >
				<label>用户编号:</label> 
				<input type="text" class="form-control" name="usersId" value="${user.usersId==0?'':user.usersId }" placeholder="请输入用户编号" />
				<label>用户姓名:</label>
				<input type="text" class="form-control" name="usersName" value="${user.usersName }"  placeholder="请输入用户姓名" />
			</div>
			<input type="submit" class="btn btn-danger" value="查询"> <a
				href="system/addUser" class="btn btn-success">添加用户</a>
			<input type="button" class="btn btn-warning" id="del" value="删除用户">
		</div>
		<div class="row" style="padding: 15px;">
			<d:table class="table table-hover table-condensed" name="usersList" pagesize="3" requestURI="system/queryUsers">
				...
				<d:column property="usersSalsy" title="用户薪资"></d:column>
				<d:column value="修改" href="system/updateUser" paramId="usersId" paramProperty="usersId"></d:column>
